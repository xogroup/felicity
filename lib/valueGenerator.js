...
internals.getType = function (schema) {
    ...
    if (Examples[exampleType] === undefined) {
        ...
        if (schemaDescription.metas && schemaDescription.metas[0].baseType) {
            exampleType = schemaDescription.metas[0].baseType;
        }
    }
    ...
    _generate(rules) {
        ...
        if (schemaDescription.matches.length > 1) {
            ...
            if (schemaDescription.matches[0].ref) {
                const driverPath = schemaDescription.matches[0].ref.path[0];
                const driverValue = Hoek.reach(this._hydratedParent, driverPath);

                let driverIsTruthy = false;

                const { error } = Helpers.descriptionCompiler(schemaDescription.matches[0].is).validate(driverValue);
