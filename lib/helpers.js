...
internals.ruleDictionaryBuilder = function (schemaDescriptionRules, transformRuleFunc) {

    const options = {};

    schemaDescriptionRules.forEach((rule) => {

        if (transformRuleFunc) {
            transformRuleFunc(rule);
        }
...
internals.string = function (schema, options) {
    ...
    if (Hoek.reach(schemaDescription, 'valids')) {
        ...
        const stringOptions = internals.ruleDictionaryBuilder(schemaDescription.rules);
...
internals.number = function (schema, options) {
    ...
    if (Hoek.reach(schemaDescription, 'flags.allowOnly')) {
        ...
        const numberOptions = internals.ruleDictionaryBuilder(schemaDescription.rules, transformRuleFunc);
...
internals.binary = function (schema) {
    ...
    if (schemaDescription.rules) {
        const bufferOptions = internals.ruleDictionaryBuilder(schemaDescription.rules);
...
internals.date = function (schema) {
    ...
    if (schemaDescription.rules) {
        ...
        const options = internals.ruleDictionaryBuilder(schemaDescription.rules);
        let min = 0;
...
internals.array = function (schema) {
    ...
    if (!arrayIsSparse) {
        ...
        if (schemaDescription.rules) {
            const arrayOptions = internals.ruleDictionaryBuilder(schemaDescription.rules);
...
internals.object = function (schema, options) {
    ...
    if (schemaDescription.rules) {
        const objectOptions = internals.ruleDictionaryBuilder(schemaDescription.rules);
